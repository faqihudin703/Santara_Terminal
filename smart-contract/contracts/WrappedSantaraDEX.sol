...
/**
 * @title WrappedSantaraDEX
 * @dev Simple AMM DEX (ETH <-> wSAN Token)
 * - Upgradeable (Transparent Proxy)
 * - Fees enabled (Revenue Model)
 * - LP Tokens (wSLP)
 */
contract WrappedSantaraDEX is
    Initializable,
    ERC20Upgradeable,
    ...
    PausableUpgradeable
{
    ...
    IERC20Upgradeable public token; // Wrapped Santara Token (wSAN)
    ...
    ) external initializer
    {
        ...
        // Nama LP Token: Wrapped Santara LP Token (wSLP)
        __ERC20_init("Wrapped Santara LP Token", "wSLP");
